...
def gaussian_box(n: int, pk: Callable[[np.ndarray], np.ndarray], seed: int) -> np.ndarray:
    ...
    ``vol_normalised_power=True``) already use exactly this convention once ``size`` is
    given in cells.

    ``powerbox`` 1.0 draws from its own generator, seeded from ``seed`` alone, so the
    box does not depend on what ran before it and ``seed=0`` behaves like any other.
    Earlier versions needed the global stream seeded by hand to get either property,
    which is why ``powerbox>=1.0`` is required.
    """
    pbx = pytest.importorskip("powerbox")
    box = pbx.PowerBox(shape=(n,) * 3, pk=pk, size=(float(n),) * 3, seed=seed)
    return np.asarray(box.delta_x())
